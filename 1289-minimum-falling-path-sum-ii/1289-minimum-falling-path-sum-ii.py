class Solution:
    def minFallingPathSum(self, grid: List[List[int]]) -> int:
        ...
        def optimal(row, col):
            ...
            # If the result of this sub-problem is already catched
            if (row, col) in memo:
                return memo[(row, col)]
            
            # Select grid[row][col], and move on to next row.
            # For next row, choose the cell that leads to the minimum sum
            next_minimum = inf
        ...
        # We can select any element from the first row.
        # We will select the element which leads to minimum sum.
        answer = inf
        for col in range(n):
